...
Em conflitos, aplicar a `Hierarquia de Autoridade` definida em `.agents/AGENTS.md`.

Para confirmar o estado técnico real antes de repetir uma iteração, usar as `Fontes de Verdade do Projeto` definidas em `.agents/AGENTS.md`.
...
9. Atualizar `.agents/state/TODO.md` e `.agents/state/LESSONS.md` quando aplicável.
...
- o pedido do utilizador entra em conflito com `.agents/AGENTS.md`;
...
- o diff foi revisto;
- `.agents/state/TODO.md` foi atualizado em tarefa não trivial;
- `.agents/state/LESSONS.md` foi atualizado se houve aprendizagem reutilizável;
